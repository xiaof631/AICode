...
struct TodoStatsView: View {
    ...
    @State private var showHistoryDatePicker = false // 新增：历史日期选择器状态
    ...
    init(dataManager: TodoDataManager) {
        ...
        // 使用 _startDate 和 _endDate 初始化 @State 属性
    ...
    private var taskDistributionStats: some View {
        VStack(alignment: .leading) {
            ...
            VStack(spacing: 12) {
                // 第一行：重要且紧急、重要不紧急
                HStack(spacing: 20) {
                    let importantUrgent = TaskQuadrant.important_urgent
                    let importantNotUrgent = TaskQuadrant.important_notUrgent
                ...
                // 第二行：不重要但紧急、不重要不紧急
                HStack(spacing: 20) {
                    let notImportantUrgent = TaskQuadrant.notImportant_urgent
                    let notImportantNotUrgent = TaskQuadrant.notImportant_notUrgent
    ...
    private var totalScheduledTimeText: String {
        let totalSeconds = filteredLogs.reduce(0) { $0 + $1.scheduledTasks.reduce(0) { $0 + $1.duration } }
        return formatTimeInterval(totalSeconds)
    }
    
    // 计算属性 - 实际总时长
    private var totalCompletedTimeText: String {
        let totalSeconds = filteredLogs.reduce(0) { $0 + $1.completedTasks.reduce(0) { $0 + $1.duration } }
        return formatTimeInterval(totalSeconds)
    }
    
    // 计算属性 - 时间执行率
    private var timeExecutionRateText: String {
        let scheduledTime = filteredLogs.reduce(0) { $0 + $1.scheduledTasks.reduce(0) { $0 + $1.duration } }
        if scheduledTime == 0 {
            return "0%"
        }
        
        let completedTime = filteredLogs.reduce(0) { $0 + $1.completedTasks.reduce(0) { $0 + $1.duration } }
    ...
    private var averageTaskTimeText: String {
        ...
        let totalTime = filteredLogs.reduce(0) { $0 + $1.completedTasks.reduce(0) { $0 + $1.duration } }
