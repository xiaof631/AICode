...
enum WeekType: String, CaseIterable, Identifiable {
    ...
    var id: String { self.rawValue } // 唯一标识符

    // 本地化名称
    var localizedName: String {
        switch self {
        case .bigWeek:
            return NSLocalizedString("6-Day Week", comment: "Work Monday to Saturday") // 6天工作周
        case .smallWeek:
            return NSLocalizedString("5-Day Week", comment: "Work Monday to Friday") // 5天工作周
        }
    }
}
...
enum WorkScheduleType: String, CaseIterable, Identifiable {
    ...
    var localizedName: String {
        switch self {
        case .fixed:
            return NSLocalizedString("Fixed", comment: "Fixed schedule type") // 固定
        case .alternating:
            return NSLocalizedString("Alternating", comment: "Alternating schedule type") // 交替
        case .shift:
            return NSLocalizedString("Shift", comment: "Shift schedule type") // 轮班
        case .custom:
            return NSLocalizedString("Custom", comment: "Custom schedule type") // 自定义
        }
    }
}
...
enum WorkStatus: String, CaseIterable, Identifiable {
    ...
    case dinnerBreak = "Dinner Break" // 新增晚餐状态
    ...
    // 提供本地化的状态名称 (如果需要的话)
    var localizedName: String {
        NSLocalizedString(self.rawValue, comment: "Work status")
    }
}
...
protocol WorkSchedule {
    ...
    var dinnerStartTime: Date { get set }     // 新增：晚餐开始时间
    var dinnerEndTime: Date { get set }       // 新增：晚餐结束时间
...
struct FixedWorkSchedule: WorkSchedule {
    ...
    func isWorkDay(date: Date) -> WorkDayType {
        ...
        let weekday = calendar.component(.weekday, from: date) // 获取星期几

        // 检查是否为公共节假日
        if HolidayManager.shared.isHoliday(date: date) {
            return .holiday
        }

        // 1 = 周日, 2 = 周一, ..., 7 = 周六
        // 如果是周一到周五，则为工作日，否则为休息日
        return (weekday >= 2 && weekday <= 6) ? .workDay : .restDay
    }
    ...
    private func checkWorkTimeStatus(date: Date) -> WorkStatus {
        ...
        // 使用结构体自身的晚餐时间属性
        let dinnerStart = calendar.dateComponents([.hour, .minute], from: dinnerStartTime) // 修改
        let dinnerEnd = calendar.dateComponents([.hour, .minute], from: dinnerEndTime)     // 修改
        ...
        if nowMinutes < workStartMinutes || nowMinutes >= workEndMinutes {
            ...
            return .lunchBreak // 午休时间内 -> 午休中
             ...
             // 工作时间内的晚餐判断（如果允许）
             // 根据实际需求，晚餐时间可能只在下班后计算，或者工作时间内也算？
             // 这里假设工作时间内也判断晚餐
             return .dinnerBreak
        } else {
...
struct AlternatingWorkSchedule: WorkSchedule {
    ...
    func isWorkDay(date: Date) -> WorkDayType {
        ...
        let targetWeekOfYear = calendar.component(.weekOfYear, from: targetDay)
        // 简单的周差计算，可能需要处理跨年的情况
        // 更健壮的方式是计算两个日期之间的完整周数
        let weekDifference = calendar.dateComponents([.weekOfYear], from: today, to: targetDay).weekOfYear ?? 0
        ...
        if currentWeekType == .bigWeek {
            isTargetWeekBig = (weekDifference % 2 == 0)
        } else {
            isTargetWeekBig = (weekDifference % 2 != 0)
        }
    ...
    private func checkWorkTimeStatus(date: Date) -> WorkStatus {
        ...
        let dinnerStart = calendar.dateComponents([.hour, .minute], from: dinnerStartTime) // Add this line
        let dinnerEnd = calendar.dateComponents([.hour, .minute], from: dinnerEndTime)     // Add this line
        ...
        let lunchStartMinutes = (lunchStart.hour ?? 0) * 60 + (lunchStart.minute ?? 0)
        let lunchEndMinutes = (lunchEnd.hour ?? 0) * 60 + (lunchEnd.minute ?? 0)
        let dinnerStartMinutes = (dinnerStart.hour ?? 0) * 60 + (dinnerStart.minute ?? 0) // Add this line
        let dinnerEndMinutes = (dinnerEnd.hour ?? 0) * 60 + (dinnerEnd.minute ?? 0)     // Add this line
...
struct ShiftWorkSchedule: WorkSchedule {
    ...
    func isWorkDay(date: Date) -> WorkDayType {
        ...
        // 计算目标日期距离轮班开始日期的天数
        // 确保比较的是日期部分，忽略时间
        let cycleStartDate = calendar.startOfDay(for: startDate)
        let targetDate = calendar.startOfDay(for: date)
        let daysSinceStart = calendar.dateComponents([.day], from: cycleStartDate, to: targetDate).day ?? 0
        ...
        guard cycleLength > 0 else { return .restDay }
    ...
    private func checkWorkTimeStatus(date: Date) -> WorkStatus {
        ...
        let lunchEnd = calendar.dateComponents([.hour, .minute], from: lunchBreakEndTime)
        // 获取晚餐时间
        let dinnerStart = calendar.dateComponents([.hour, .minute], from: WorkScheduleManager.shared.dinnerStartTime)
        let dinnerEnd = calendar.dateComponents([.hour, .minute], from: WorkScheduleManager.shared.dinnerEndTime)
